...
describe('AuthService', () => {
        ...
        jwtMock = {
            sign: jest.fn().mockImplementation((payload, secret, options) => {
                if (payload.userId === 'user123') {
                    return secret === 'secret' ? 'mock_access_token' : 'mock_refresh_token';
                }
                return 'token';
            }),
            verify: jest.fn().mockImplementation((token, secret) => {
    ...
    describe('login', () => {
        it('should log in a user successfully', async () => {
            ...
            expect(bcryptMock.compare).toHaveBeenCalledWith(credentials.password, 'hashed_password');
            expect(jwtMock.sign).toHaveBeenCalledTimes(2);
            expect(prismaMock.refreshToken.create).toHaveBeenCalled();
            expect(result).toEqual({
                user: {
    ...
    describe('refreshToken', () => {
        it('should refresh tokens successfully', async () => {
            ...
            // Assert
            expect(bcryptMock.verify).toHaveBeenCalledWith(refreshToken, expect.any(String));

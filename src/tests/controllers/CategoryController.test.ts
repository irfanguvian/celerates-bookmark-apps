...
describe('CategoryController', () => {
    ...
    describe('getAllCategories', () => {
        it('should successfully get all categories', async () => {
            ...
            (categoryService.getAllCategories as jest.Mock).mockResolvedValue(serviceResponse);

            // Act
            const result = await categoryController.getAllCategories(mockContext as Context);

            // Assert
            expect(categoryService.getAllCategories).toHaveBeenCalledWith('user123', queryParams);
    ...
    describe('createCategory', () => {
        it('should successfully create a category', async () => {
            ...
            (categoryService.createCategory as jest.Mock).mockResolvedValue(serviceResponse);

            // Act
            const result = await categoryController.createCategory(mockContext as Context);

            // Assert
            expect(categoryService.createCategory).toHaveBeenCalledWith('user123', categoryData);
            expect(successResponse).toHaveBeenCalledWith(
                mockContext,
    ...
    describe('updateCategory', () => {
        it('should successfully update a category', async () => {
            ...
            const serviceResponse = { id: categoryId, ...updateData, description: 'Test description' };

            mockReq.param.mockReturnValue(categoryId);
            mockReq.json.mockResolvedValue(updateData);
            (categoryService.updateCategory as jest.Mock).mockResolvedValue(serviceResponse);

            // Act
            const result = await categoryController.updateCategory(mockContext as Context);

            // Assert
            expect(categoryService.updateCategory).toHaveBeenCalledWith('user123', categoryId, updateData);
            expect(successResponse).toHaveBeenCalledWith(
                mockContext,
                { category: serviceResponse },
    ...
    describe('deleteCategory', () => {
        it('should successfully delete a category', async () => {
            ...
            // Act
            const result = await categoryController.deleteCategory(mockContext as Context);

            // Assert
            expect(categoryService.deleteCategory).toHaveBeenCalledWith('user123', categoryId);
            expect(successResponse).toHaveBeenCalledWith(
                mockContext,
                {},
